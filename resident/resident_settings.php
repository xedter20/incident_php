...
if (isset($_POST['updateInfo'])) {
    ...
    $currentData = $db->resident_data($residentID);
    ...
    if ($fname !== $currentData['first_name'] ||
        ...
        $stmt = $db->UPDATE_resident_data_Information($fname, $mname, $lname, $c_address, $cp_number, $residentID);
...
if (isset($_POST['updatePword'])) {
    ...
    $password = $currentData['resident_password'];

    if ($pword == $password) {
        if ($npword == $rnpword) {

            // Prepare and execute the SQL update query
        $res = $db->UPDATE_resident_data_password($npword, $residentID);
...
<form action="" method="POST" class="row g-3 needs-validation" novalidate="">
  <div class="col-md-4 position-relative">
    ...
    <input type="hidden"name="residentID"value="<?php echo $data['id']; ?>" required="">
