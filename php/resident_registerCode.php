...
if (isset($_POST['register'])) {
    ...
    $uniqueFilename = uniqid() . '-' . $_FILES['resident_pic']['name'];

    // Define the full path to the saved image file
    $imagePath = $uploadDirectory . $uniqueFilename;

    $user = $db->resident_email($email); # get user data
    ...
    if ($user) {
        ...
        header("location: ../resident/resident_register.php");
        exit();
    }

    $userNumber = $db->resident_phoneNUMBER($PhoneNumber);
    ...
    if ($pword == $c_pword) {
        ...
        if (move_uploaded_file($_FILES['resident_pic']['tmp_name'], $imagePath)) {

            $sql = $db->resident_REGISTER_INSERT($uniqueId, $first_name, $middle_name, $last_name, $bDay, $age, $gender, $CompleteAddress, $PhoneNumber, $email, $pword, $imagePath, $verification_code, $latitude, $longitude);
